...
def authenticate(response: Response, user: UserLogin, session: Session = Depends(get_session)):
    user = UserLogin.model_validate(user)

    logged_user = AuthService().authenticate_user(user,session)
    
    response.set_cookie('session_cfs_tkn_ath', logged_user.jwt, expires=logged_user.expires_date.strftime("%d/%m/%Y, %H:%M:%S"), httponly=True)
